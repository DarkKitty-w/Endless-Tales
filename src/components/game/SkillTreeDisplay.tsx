...
import type { SkillTree, Skill } from "../../types/game-types";
import { CardboardCard, CardContent, CardHeader, CardTitle } from "../../components/game/CardboardCard";
import { ScrollArea } from "../../components/ui/scroll-area";
import { Badge } from "../../components/ui/badge";
import { Button } from "../../components/ui/button";
import { Star, Lock, Unlock, Sparkles, CheckCircle2, CircleDot, Workflow, Wand2 } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
      ...
      <CardboardCard className="m-4">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Workflow className="w-5 h-5" /> Skill Tree Error
          </CardTitle>
        </CardHeader>
        <CardContent>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Badge variant="secondary" className="cursor-help text-xs py-0.5 px-1.5">
                        {skill.name} {skill.type === 'Starter' ? '(S)' : ''}
                      </Badge>
                    </TooltipTrigger>
                    <TooltipContent side="top" align="start" className="max-w-xs">
                      <p className="font-semibold">{skill.name} {skill.type === 'Starter' ? '(Starter)' : ''}</p>
                      <p className="text-xs text-muted-foreground">{skill.description}</p>
                      {(skill.staminaCost !== undefined || skill.manaCost !== undefined) && (
                        <p className="text-xs mt-1">
                          Cost: {skill.staminaCost && <span className="text-green-600">{skill.staminaCost} STA </span>}
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <Tooltip>
                      ...
                      <TooltipContent side="top">
                        Stage {stageData.stage}: {stageData.stageName}
                      </TooltipContent>
                    </Tooltip>
                    {isCurrent && (
                      <Badge variant="default" className="text-xs ml-auto mr-2 py-0 px-1.5 h-5">
                        Current
                      </Badge>
                    )}
                  </div>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="flex items-center gap-0.5">
                          ...
                          {[...Array(maxStage - stageData.stage)].map((_, i) => (
                            <Star key={`empty-${i}`} className="w-3 h-3 text-muted-foreground/30" />
                          ))}
                        </span>
                      </TooltipTrigger>
                      <TooltipContent side="top">Stage {stageData.stage} / {maxStage} Achieved</TooltipContent>
                    </Tooltip>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="pt-3 pl-5 pr-3 pb-4 bg-background">
                  {isUnlocked || stageData.stage === 0 ? (
                    stageData.skills && stageData.skills.length > 0 ? (
                          ...
                          return (
                            <li key={skill.name} className="text-sm flex items-start gap-2">
                              {isLearned ? (
                                ...
                                <CircleDot className="w-3.5 h-3.5 text-muted-foreground/50 flex-shrink-0 mt-0.5" />
                              )}
                                  ...
                                  <TooltipTrigger asChild>
                                    <span
                                      className={`font-medium ${
                                        isLearned ? 'cursor-help' : 'text-muted-foreground italic cursor-help'
                                      }`}
                                    >
                                      {skill.name}
                                      {isLearned && (
                                        <span className="text-green-600 font-normal text-xs"> (Learned)</span>
                                      )}
                                    </span>
                                  </TooltipTrigger>
                      ...
                      <p className="text-sm text-muted-foreground italic">
                        No specific new skills become available at this stage.
                      </p>
                    ) : (
                      <p className="text-sm text-muted-foreground italic">
                        This is the starting point. Advance further to unlock skills.
                      </p>
                    )
                  ) : (
                    <p className="text-sm text-muted-foreground italic">
                      Locked. Progress further to unlock this stage.
                    </p>
                  )}
                </AccordionContent>
              </AccordionItem>
