...
import { Loader2, Settings, ArrowLeft, Skull, Save, Info, Dices } from "lucide-react"; // Added Info, Dices
...
import { BookCopy, CalendarClock, GitBranch, Hammer } from "lucide-react"; // Added for NarrationDisplay

// --- Dice Roller Service (Embedded for standalone functionality if services/dice-roller.ts is problematic) ---
// Simpler dice roller if external service is an issue
...
export function Gameplay() {
    ...
    const [localIsGeneratingSkillTree, setLocalIsGeneratingSkillTree] = useState(contextIsGeneratingSkillTree);
    ...
    const triggerSkillTreeGeneration = useCallback(async (charClass: string) => {
        if (!charClass || localIsGeneratingSkillTree || adventureSettings.adventureType === "Immersed") {
    ...
    }, [dispatch, toast, localIsGeneratingSkillTree, adventureSettings.adventureType]);
    ...
    const handlePlayerAction = useCallback(async (action: string, isInitialAction = false) => {
        ...
        if (character.class === 'admin000') {
            setIsLoading(true); setError(null); setDiceResult(null); setDiceType("None"); setBranchingChoices([]);
        ...
        let rollFunction: (() => number) | null = null; // Changed to synchronous
        ...
        if (!isInitialAction && !isPassiveAction) {
            ...
            try {
                ...
                if (assessedDifficulty === "Impossible") {
                    ...
                    setIsLoading(false); setIsAssessingDifficulty(false); return;
                }
                ...
                assessedDifficulty = "Normal"; setDiceType("d10"); rollFunction = localGetDiceRollFunction("d10");
                requiresRoll = !!rollFunction;
            } finally { setIsAssessingDifficulty(false); }
        } else {
        ...
        if (requiresRoll && rollFunction) {
            ...
            await new Promise(resolve => setTimeout(resolve, 200));
            try {
                const roll = rollFunction(); // Synchronous call
                setDiceResult(roll);
        ...
        while (retryCount <= maxRetries && !narrationResult) {
            try {
                ...
                if (!result.updatedGameState.toLowerCase().includes('turn:')) throw new Error("AI response missing Turn count in updated game state.");
        ...
        if (narrationResult) {
            ...
            if (narrationResult.isCharacterDefeated) {
                if (adventureSettings.permanentDeath) {
                    ...
                    // Non-permadeath, trigger respawn
         ...
         setIsLoading(false); if (isInitialAction) setIsInitialLoading(false);
    }, [
    ...
    const handleCrafting = useCallback(async (goal: string, ingredients: string[]) => {
     ...
     const handleConfirmClassChange = useCallback(async (newClass: string) => {
         ...
         setPendingClassChange(null); dispatch({ type: "SET_SKILL_TREE_GENERATING", payload: true });
    ...
    useEffect(() => {
        ...
        if (isInitialLoading) {
             if (adventureSettings.adventureType !== "Immersed" && !character.skillTree && !localIsGeneratingSkillTree) {
                 triggerSkillTreeGeneration(character.class);
        ...
        state.status, character, storyLog.length, localIsGeneratingSkillTree,
        triggerSkillTreeGeneration, handlePlayerAction, isInitialLoading, isLoading, adventureSettings.adventureType
    ]);
