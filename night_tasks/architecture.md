...
### ARCH-1: God File - AI Router
**Severity:** High  
**Description:** `ai-router.ts` is 2072 lines and ~66KB, handling AI provider configuration, multiple provider implementations (Gemini, OpenAI, Claude, DeepSeek, WebLLM), request routing, streaming logic, error handling, prompt injection protection, observability logging, timeout handling, and WebLLM engine management. This violates the Single Responsibility Principle.  
**Location:** `src/ai/ai-router.ts`  
**Problem:** Monolithic file that is difficult to maintain, test, or extend. A bug in one provider's implementation can affect others. The file has grown unchecked and mixes high-level routing with low-level provider details.  
**Recommendation:** Split into modular components:
- `src/ai/providers/base-provider.ts` (AIProvider interface)
- `src/ai/providers/gemini-provider.ts`, `openai-provider.ts`, etc. (individual provider implementations)
- `src/ai/ai-router.ts` (lean routing logic only)
- `src/ai/ai-utils.ts` (shared utilities: timeout, logging, prompt protection)
- `src/ai/webllm-manager.ts` (WebLLM engine lifecycle management)

### ARCH-2: God File - GameContext
**Severity:** High  
...
- `src/context/GameProvider.tsx` (provider component with nested domain providers)
- `src/context/domain-contexts.ts` (domain context definitions and hooks)
- `src/context/game-migrations.ts` (save migration logic)
- `src/context/game-persistence.ts` (localStorage load/save logic)
- `src/context/theme-manager.ts` (theme application logic)

### ARCH-3: Deprecated Context Usage Causing Unnecessary Re-renders
**Severity:** High  
...
### ARCH-4: Tight Coupling Between UI and AI Logic
**Severity:** Medium  
...
**Recommendation:** Create an AI service layer:
- Create `src/services/ai-service.ts` that wraps all AI flow calls
...
### ARCH-5: Fragmented Multiplayer State Management
**Severity:** Medium  
...
### ARCH-6: Side Effects in Reducer
**Severity:** Medium  
...
### ARCH-7: Unused Domain Context Abstraction
**Severity:** Low  
...
### ARCH-8: Legacy Utilities in game-state-utils
**Severity:** Low  
...
**Recommendation:**
1. Split into focused utility files: `src/context/state-string-builder.ts`, `src/context/game-context-builder.ts`
