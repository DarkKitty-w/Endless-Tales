...
4.  **Adventure End and Story Saving**:
    *   **End of Adventure**: When the player’s journey ends (via death, success, or failure), the game automatically processes the ending and presents a summary screen.
    *   **Simple or Detailed Story**: Players can view a high-level summary generated by the AI or a more detailed narrative breakdown (the full story log) on the summary screen.
    *   **Automatic Saving (Session)**: The game saves the final story log and summary to the application's state when the adventure ends. This allows players to review their completed journey on the summary screen *within the current session*. (Note: Persistent saving to view past adventures across sessions is not implemented yet).
